...
X = pickle.load(pickle_in)/255

pickle_in2 = open("y2.pickle","rb")
...
model_final.compile(loss = "sparse_categorical_crossentropy", optimizer = optimizers.Adam(lr=0.001), metrics=["accuracy"], options = run_opts)
...
# Train the model 
StepsPerEpoch = int(np.ceil(numTrainSam/MyBatchSize))
ValStepsPerEpoch = int(np.ceil(ValSize/MyBatchSize))
...
# model_final.fit(Xtrain,Ytrain, epochs = Epochs, steps_per_epoch = StepsPerEpoch, verbose=1, validation_data=(Xval,Yval), validation_steps = ValStepsPerEpoch, callbacks = [checkpoint])
model_final.fit(Xtrain,Ytrain, epochs = Epochs, batch_size = MyBatchSize, verbose=1, validation_data=(Xval,Yval), callbacks = [checkpoint])
